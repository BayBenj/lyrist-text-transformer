...
public abstract class LyristTransformer {
	...
	private static final int N_NORMAL = 25;
	private static final int N_RHYME = 500;
...
//        U.testPrint("Entered cosineStringsToWords");
		...
		for (Map.Entry<Double, Word> entry : wordMap.entrySet()) {
			...
			temp.setCosineSimilarity(entry.getKey());
			result.add(temp);
		}
		return result;
	}
		...
		for (Map.Entry<Word, Set<Word>> wordToSuggestions : wordsToSuggestions.entrySet()) {
			...
			try {
				...
				if (rhyming && oldWordsByRhyme.contains(wordToSuggestions.getKey())) {
					chosen = WordConstraintRunner.useConstraintsTo1ByWeakening(((RhymeTransformInfo)info).getRhymeConstraints(), oldWord, suggestions, new ProbabalisticParams(1, 100));
				}
				else {
					//Use normal filters
					...
					if (baseReplacements.keySet().contains(oldWord.getBase())) {
						...
						chosen = WordConstraintRunner.useConstraintsTo1ByWeakening(info.getBaseConstraints(baseReplacements.get(oldWord.getBase())), oldWord, suggestions, new ProbabalisticParams(1, 0));
					}
					else
						chosen = WordConstraintRunner.useConstraintsTo1ByWeakening(info.getNormalConstraints(), oldWord, suggestions, new ProbabalisticParams(1, 0));
				}
