...
public class NHMM<T> extends AbstractMarkovModel<T>{
    ...
    public NHMM(SingleOrderMarkovModel<T> model, int length, List<Pair<Integer, PaulConstraint<T>>> constraints) {
...
//		System.out.println(this);
        ...
        for (Pair<Integer, PaulConstraint<T>> constraint : constraints) {
...
//			str.append((logPriors[i]));
    ...
    public void constrain(Pair<Integer, PaulConstraint<T>> positionedConstraint) {
        Set<PositionedState> posStateToRemove = new HashSet<PositionedState>();
        int position = positionedConstraint.getFirst();
        PaulConstraint<T> constraint = positionedConstraint.getSecond();
        position = (position == PaulConstraint.FINAL_POSITION ? logTransitions.length : position);
        PaulConstraintCondition<T> condition = constraint.getCondition();
